...
const handlePOST = async (req: NextApiRequest, res: NextApiResponse) => {
  ...
  if (from === 'admin') {
    userTeam = await getTeam({ slug: teamSlug });
    await addTeamMember(
      userTeam.id,
      user.id as string,
      Role.MEMBER, //HARDCODED ROLE HERE
      teamRoleId
    );
  } else if (!invitation) {
    ...
    const teamRoleAdminId = await createAdminRole(userTeam.id);

    await addTeamMember(userTeam.id, user.id, Role.OWNER, teamRoleAdminId.id);
  } else {
