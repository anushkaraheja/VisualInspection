...
const UpdateTheme = () => {
  const { setTheme, themes, selectedTheme } = useTheme();
  ...
  return (
    <Card>
      <Card.Body>
        ...
        <div className="relative dropdown w-60 bg-surfaceColor dark:bg-black">
          <div
            tabIndex={0}
          ...
          <ul
            tabIndex={0}
            className="absolute z-10 dropdown-content dark:border-gray-600 dark:bg-backgroundColor p-2 shadow-md bg-base-100 w-full rounded border px-2 mt-1"
          >
            {themes.map((theme) => (
              <li key={theme.id}>
                <button
