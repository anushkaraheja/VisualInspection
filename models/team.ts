...
import { Resource, Role, Team } from '@prisma/client';
import type { NextApiRequest, NextApiResponse } from 'next';
import { getCurrentUser } from './user';
import { normalizeUser } from './user';
import { validateWithSchema, teamSlugSchema } from '@/lib/zod';
import { ActionEnumFlags, addAction } from '@/lib/permissions';
...
export const addTeamMember = async (
  ...
  if (teamRoleId === '') {
    const teamMemberRoleId = await getMemberRoleID(teamId);
    if (teamMemberRoleId) {
      teamRoleId = teamMemberRoleId.id;
    } else {
      throw new Error('Team member role ID not found');
    }
  }
...
export const createAdminRole = async (teamId) => {
  // Create Admin Role linked to existing Team
  ...
  //created a default member role with each team
  ...
  await prisma.rolePermission.create({
    data: {
      ...
      action: addAction(0, ActionEnumFlags.ALL),
    },
  });

  //added some default permissions to member role
  await prisma.rolePermission.create({
    data: {
      teamRoleId: memberRole.id,
      resource: Resource.TEAM,
      action: addAction(0, ActionEnumFlags.READ | ActionEnumFlags.LEAVE),
    },
  });
  ...
  await prisma.rolePermission.create({
    data: {
      ...
      action: addAction(0, ActionEnumFlags.READ),
    },
  });
...
export async function getMemberRoleID(teamId: string) {
  if (!teamId) {
    throw new Error('Team ID is required to create roles');
  }

  return await prisma.teamRole.findFirst({
    where: {
      teamId,
      name: 'Member',
    },
